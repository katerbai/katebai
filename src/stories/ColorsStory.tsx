        ...
        <div className="w-full space-y-2">
          {PROJECT_COLORS.map((c) => (
            <div key={c.label} className="flex items-center gap-4 rounded-xl border border-border/50 bg-card px-5 py-3">
              <div className="h-10 w-10 flex-shrink-0 rounded-lg border border-border/30" style={{ background: c.swatch }} />
              ...
              <code className="font-mono text-xs text-primary bg-primary/10 px-2.5 py-1 rounded-full">{c.value}</code>
            </div>
          ))}
        </div>
      </StoryBlock>
